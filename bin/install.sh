...
wget -q -P ~/${BASE_DIR}/ https://download.elastic.co/elasticsearch/release/org/elasticsearch/distribution/tar/elasticsearch/2.3.3/elasticsearch-2.3.3.tar.gz
tar -xvzf ~/${BASE_DIR}/elasticsearch-2.3.3.tar.gz -C ~/${BASE_DIR} > /dev/null

wget -q -P ~/${BASE_DIR} https://download.elastic.co/kibana/kibana/kibana-4.5.3-linux-x64.tar.gz
tar -xvzf ~/${BASE_DIR}/kibana-4.5.3-linux-x64.tar.gz -C ~/${BASE_DIR} > /dev/null
...
# Start Kibana
nohup ~/${BASE_DIR}/kibana-4.5.3-linux-x64/bin/kibana &
